...
Partial Class Maintenance_LocationList
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        employeeMstrId = Request.Params("employeeMstrId")
    ...
    Protected Sub bindSelectionList()
        ...
        Try
            If employeeMstrId = "" And Not Session("tempLocation") Is Nothing Then
                ...
                sql = "select locationinfoid as value,locationName as display from locationinfo where active = 'Y' order by locationname"
            ...
            ElseIf Session("tempLocation") Is Nothing And employeeMstrId = "" Then
                sql = "select locationinfoid as value,locationName as display from locationinfo where active = 'Y' order by locationname"
                'Load Available Location
                dsLocation.SelectCommand = sql
                lstLocation.DataTextField = "Display"
            ...
            Else
                ...
                sql = "SELECT LI.LOCATIONINFOID as Value,LI.LOCATIONNAME AS Display FROM EMPLOYEELOCATION EL,LOCATIONINFO LI " & _
                      "WHERE(EL.LOCATIONINFOID = LI.LOCATIONINFOID) AND EL.EMPLOYEEMSTRID = " & employeeMstrId
                ...
                sql = "select locationinfoid as value,locationName as display from locationinfo where locationinfoid not in " & _
                      "(select locationinfoid from employeelocation where employeemstrid = " & employeeMstrId & ") order by locationname"
                'Load Available Location
                dsLocation.SelectCommand = sql
                lstLocation.DataTextField = "Display"
                lstLocation.DataValueField = "Value"
    ...
    Protected Sub btnAddItem_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddItem.Click
        ...
        For i = lstLocation.Items.Count - 1 To 0 Step -1
            If lstLocation.Items(i).Selected = True Then
                ...
                lstSelection.Items.Add(li)
                lstLocation.Items.Remove(li)
                If employeeMstrId <> "" Then
                    InsertRecords(li)
                Else
                    'Set In to Session
                    Session("tempLocation") = lstSelection
                End If
            End If
        Next i
    ...
    Protected Sub removeRecords(ByVal li As ListItem)
        ...
        Dim sql As String = "DELETE EMPLOYEELOCATION WHERE LOCATIONINFOID = " & id & " AND EMPLOYEEMSTRID = " & employeeMstrId
    ...
    Protected Sub InsertRecords(ByVal li As ListItem)
        ...
        Try
            empLocEnt.setLocationInfoId(li.Value.Split("/")(0))
            empLocEnt.setEmployeeMstrId(employeeMstrId)
